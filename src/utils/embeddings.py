...
try:
    from .specialized_embeddings import get_specialized_embedding_manager
...
class EmbeddingsManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized EmbeddingsManager with model: {model_name}, device: {self.device}"
        )
    ...
    def model(self) -> SentenceTransformer:
        ...
        if self._model is None:
            ...
            try:
                ...
                logger.info(
                    f"Model loaded successfully. Dimension: {self._model_dimension}"
                )
    ...
    def benchmark(
        self, num_texts: int = 100, text_length: int = 100
    ) -> Dict[str, float]:
        """
        ...
        for _ in range(num_texts):
            text = "".join(
                random.choices(
                    string.ascii_letters + string.digits + " ", k=text_length
                )
            )
            texts.append(text)
        ...
        )  # Trigger encoding for benchmark
        end_time = time.time()
...
class UnifiedEmbeddingsManager:
    ...
    def encode(
        self, texts: Union[str, List[str]], content_type: Optional[str] = None, **kwargs
    ) -> np.ndarray:
        """
        ...
        if self.use_specialized:
            ...
            result = self.manager.encode(texts, content_type=content_type, **kwargs)
            logger.debug(f"UnifiedEmbeddingsManager: Encoded with content_type='{content_type}', shape={result.shape}")
            return result
        else:
            ...
            if hasattr(self.manager, "encode_batch"):
                ...
                if isinstance(texts, str):
                    texts = [texts]
                result = np.array(self.manager.encode_batch(texts))
            else:
                result = self.manager.encode(texts, **kwargs)
            logger.debug(f"UnifiedEmbeddingsManager: Encoded with single model, shape={result.shape}")
            return result
...
) -> Union[EmbeddingsManager, UnifiedEmbeddingsManager]:
    """
