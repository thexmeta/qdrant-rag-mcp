...
class CodeHierarchy:
    ...
    def add_method(
        ...
        method_info = {"name": method_name, "signature": signature, "summary": summary}

        if class_name and class_name in self.files[file_path]["classes"]:
            # Add to class methods
            if (
                method_name
                not in self.files[file_path]["classes"][class_name]["methods"]
            ):
                self.files[file_path]["classes"][class_name]["methods"].append(
                    method_name
                )
        else:
...
class QueryIntentClassifier:
    ...
    def __init__(
        ...
        self.collection_type = (
            collection_type  # "code", "documentation", or None for general
        )
    ...
    def classify(self, query: str) -> QueryIntent:
        ...
        self.logger.debug(
            f"Query intent classification: {best_level} ({confidence:.2f})",
            extra={
                "query": query[:50],
                "level": best_level,
        ...
        return QueryIntent(
            level=best_level, exploration_type=exploration_type, confidence=confidence
        )
...
class SemanticCache(LRUMemoryCache):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, embedding_model=None):
        ...
        component_limits = memory_config.get("component_limits", {}).get(
            "progressive_cache", {}
        )

        max_memory = float(
            component_limits.get("max_memory_mb", config.get("max_memory_mb", 200))
        )
        max_items = int(
            component_limits.get("max_items", config.get("max_cache_size", 100))
        )
    ...
    def _get_embedding_model(self):
        ...
        if self.embedding_model is None:
            raise ValueError(
                "Embedding model not set. Call set_embedding_model() first."
            )
    ...
    def _get_embedding(self, text: str) -> np.ndarray:
        ...
        if (
            ...
            if "_code" in text or "function" in text.lower() or "class" in text.lower():
                ...
                or "yaml" in text.lower()
                or "json" in text.lower()
            ):
                content_type = "config"
            elif (
                "_documentation" in text
                or "doc" in text.lower()
                or "readme" in text.lower()
            ):
        ...
        if len(self.embeddings_cache) > self.max_items:
            ...
            items_to_remove = list(self.embeddings_cache.keys())[
                : len(self.embeddings_cache) // 4
            ]
    ...
    def get_similar(self, query: str, level: str) -> Optional[ProgressiveResult]:
        ...
        if best_match and best_key:
            ...
            self.logger.info(
                f"Cache hit for query with similarity {best_similarity:.3f}",
                extra={
                    "query": query[:50],
                    "level": level,
            ...
            for key, value in cache_data.items():
                ...
                result = ProgressiveResult(
                    ...
                    hierarchy=CodeHierarchy(
                        files=value["result"]["hierarchy"]["files"]
                    ),
                    expansion_options=[
                        ExpansionOption(**opt)
                        for opt in value["result"]["expansion_options"]
                    ],
...
class HierarchyBuilder:
        ...
        for file_path, file_results in file_groups.items():
            ...
            for result in file_results:
                ...
                if chunk_type == "class":
                    ...
                    hierarchy.add_class(
                        file_path, class_name, summary=class_summary, methods=methods
                    )
    ...
    def _generate_file_summary(self, file_results: List[Dict[str, Any]]) -> str:
        ...
        functions = sum(
            1 for r in file_results if r.get("chunk_type") in ["function", "method"]
        )
        ...
        summary = (
            f"Contains {', '.join(summary_parts)}" if summary_parts else "Code file"
        )
...
class ProgressiveContextManager:
    ...
    def __init__(
        self, qdrant_client, embeddings, config: Optional[Dict[str, Any]] = None
    ):
        ...
        self.query_classifier = QueryIntentClassifier(
            self.config.get("query_classification", {})
        )
    ...
    def get_progressive_context(
        ...
        if level == "auto":
            ...
            if collection_suffix == "_documentation":
                ...
                query_intent = QueryIntent(
                    level="file", exploration_type="understanding", confidence=0.9
                )
                self.logger.info(
                    f"Auto-detected context level for docs: {level}",
                    extra={
                        "query": query[:50],
                        "level": level,
                ...
                self.logger.info(
                    f"Auto-detected context level: {level}",
                    extra={
                        "query": query[:50],
                        "level": level,
        ...
        reduction_percent = (
            f"{int((1 - token_estimate / base_tokens) * 100)}%"
            if base_tokens > 0
            else "0%"
        )
    ...
    def _search_at_level(
        ...
        if collection_suffix:
            all_collections = [
                c for c in all_collections if c.endswith(collection_suffix)
            ]
        ...
        for collection in search_collections:
            # Determine content type based on collection name for specialized embeddings
            current_content_type = "general"
            if collection.endswith("_code"):
                current_content_type = "code"
            elif collection.endswith("_config"):
                current_content_type = "config"
            elif collection.endswith("_documentation"):
                current_content_type = "documentation"

            # Generate query embedding for this specific collection's model
            try:
                query_embedding_array = self.embeddings.encode(
                    query, content_type=current_content_type
                )
                # Handle both 1D and 2D arrays
                if hasattr(query_embedding_array, "ndim"):
                    if (
                        query_embedding_array.ndim == 2
                        and query_embedding_array.shape[0] == 1
                    ):
                    ...
                    query_embedding = (
                        ...
                        else query_embedding_array.tolist()
                    )
            except Exception as e:
                self.logger.error(
                    f"Failed to generate embedding for {current_content_type}: {e}"
                )
                continue
            ...
            try:
                if search_mode == "hybrid" and hybrid_searcher:
                    ...
                    for result in search_results:
                        ...
                        result_objects_map[doc_id] = (
                            result  # Store the original result object
                        )
                    ...
                    search_type = (
                        "code"
                        if collection_suffix == "code"
                        else (
                            "documentation"
                            if collection_suffix == "documentation"
                            else "general"
                        )
                    )
                    ...
                    for fused_result in fused_results[:search_limit]:
                        doc_id = (
                            fused_result.content
                        )  # doc_id is stored in content field
                        ...
                        if doc_id in result_objects_map:
                            ...
                            result.payload["vector_score"] = vector_scores_map.get(
                                doc_id, 0.0
                            )
                            result.payload["bm25_score"] = bm25_scores_map.get(
                                doc_id, 0.0
                            )
                            ...
                            if len(parts) == 2:
                                ...
                                if fetch_results:
                                    ...
                                    result.score = fused_result.combined_score
                                    # Store individual scores in payload for transparency
                                    result.payload["vector_score"] = (
                                        vector_scores_map.get(doc_id, 0.0)
                                    )
                                    result.payload["bm25_score"] = bm25_scores_map.get(
                                        doc_id, 0.0
                                    )
                ...
                try:
                    ...
                    if search_collections:
                        resolver = DependencyResolver(
                            self.qdrant_client, search_collections[0]
                        )
        ...
        if level == "file":
            ...
            for file_path, file_results in list(file_groups.items())[:n_results]:
                ...
                best_result["content"] = (
                    f"File contains {len(file_results)} relevant sections"
                )
    ...
    def _generate_summary(self, results: List[Dict[str, Any]], level: str) -> str:
        ...
        if level == "file":
            ...
            return (
                f"Found relevant content in {len(files)} file{'s' if len(files) != 1 else ''}: "
                + ", ".join(list(files)[:3])
                + ("..." if len(files) > 3 else "")
            )
            ...
            if classes:
                return (
                    f"Found {len(classes)} relevant class{'es' if len(classes) != 1 else ''}: "
                    + ", ".join(classes[:3])
    ...
    def _get_expansion_options(
        self, hierarchy: CodeHierarchy, current_level: str
...
def get_progressive_manager(
    qdrant_client, embeddings, config: Optional[Dict[str, Any]] = None
    ...
    if _progressive_manager is None:
        _progressive_manager = ProgressiveContextManager(
            qdrant_client, embeddings, config
        )
